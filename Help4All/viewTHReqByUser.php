<?php
                ...
                $q = "SELECT s.serviceName, tr.trequestId, tr.userId, u.name, u.email, tr.tlistingid, tr.PickupAddr, tr.DestAddr, tr.PickupDate, tr.PickupTime, tr.phone, tr.comments, tr.returnJour, tr.status, tl.baseCharge, tl.chargePerKM, tl.luggage, tl.description, tl.userId from transport_service_request tr inner join transport_listing tl on tr.tlistingid = tl.tlistingId inner join services s on tl.serviceType = s.serviceId inner join users u on tr.userId = u.userId WHERE tr.userId = '$s_userId'";
